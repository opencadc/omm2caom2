...
_WORKPATH = '/space/omm2cadc/archivage2018_reduc/'

# Location of where the original files are stored. /data/cpapir/data/ on the main server
#_DATAPATH = '/data/cpapir/data/'
_DATAPATH = '/data/cpapir/reductions/'

# DATAPATH for the ARCHIVE tower
#_DATAPATH = '/nasomm_mtl/Archive/cpapir/prod/'

# DATAPATH for the last reduction
#_DATAPATH = '/data/cpapir/offlimit_new_reduction/'

# Location of the logs path
#_LOGPATH = '/archives2018/logs/'
_LOGPATH = '/space/omm2cadc/archivage2018_reduc/logs/'
...
def do_me( my_docker_template, my_docker, my_night ):
   ...
   bash_cmd = "./my_reductions_clean.py "+my_night
   print(bash_cmd)
...
def get_list_of_files( night ):
    ...
    import glob
    #Getting the list of potential fits file to archive
    list_of_files = sorted(glob.glob(data_orig_path+'/'+night+'*.fits.gz'))
    #Removing the PSF files which won't be archived
    for i in reversed(list_of_files):
       if i.find('psf')>0 or i.find('PSF')>0:
...
if __name__ == '__main__':
    if len(sys.argv) <= 1:
        # no args 
        print ('usage: run_reductions_docker.py <night_number> ')
        print()
        print("Exemple: ./my_reductions_docker.py 150110")
        sys.exit(1)
    else:
